...
// Evento personalizado para confirmar eliminación de producto
...
// Evento personalizado para aplicar un descuento especial
...
let discountApplied = false; // Variable para controlar si el descuento ya se aplicó
...
function finalizePurchase() {
    ...
    // Verifica si el total supera los 100 pesos y si el descuento no se ha aplicado
    if (total > 100 && !discountApplied) {
        document.dispatchEvent(specialDiscountEvent); // Lanza el evento para aplicar el descuento
    } else {
...
document.addEventListener('specialDiscount', (event) => {
    ...
    discountApplied = true; // Marca el descuento como aplicado para evitar que se repita
    finalizePurchase(); // Llama a la función de finalizar compra después de aplicar el descuento
});
